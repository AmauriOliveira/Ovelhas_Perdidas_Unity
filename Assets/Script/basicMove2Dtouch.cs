...
public class basicMove2Dtouch : MonoBehaviour
{
    ...
    void Update()
    {
        if (Input.GetAxis("Horizontal") < 0 && EstaEspelhado)///vefirica se a tecla foi apertado e não esta espelhado
        {
            Virar();//chama o metodo de virar
        }

        else if (Input.GetAxis("Horizontal") > 0 && !EstaEspelhado)///vefirica se a tecla foi apertado e esta espelhado
        {
            Virar();//chama o metodo de virar
        }

        transform.Translate(new Vector2(velocidade * Time.deltaTime * Input.GetAxis("Horizontal"), 0));//aplica força no x


        if (Input.GetButtonDown("Jump") && SaltoLiberado)///vefirica se a tecla foi apertado e tiver liberado pulo
        {
    ...
    {
        if (col.gameObject.CompareTag("chao") && !SaltoLiberado)///muita atenção no nome da tag, deve ser a mesma usado no inspetor
        {
            SaltoLiberado = true;
        }
    }
